...
class ModifiedResNetDecoder(nn.Module):
    ...
    def __init__(self, layers, input_dim, output_dim=3, width=64):
        ...
        self.conv3 = nn.ConvTranspose2d(width, width // 2, kernel_size=3,padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(width // 2)
        self.conv2 = nn.ConvTranspose2d(width // 2, width // 2, kernel_size=3,padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(width // 2)
        self.conv1 = nn.ConvTranspose2d(width // 2, output_dim, kernel_size=4, stride=2, padding=1, bias=False)
    ...
    def forward(self, z):
        ...
        z = self.upsample(z)  ####torch.Size([64, 64, 128, 64])
        z = self.relu(self.bn3(self.conv3(z))) #torch.Size([64, 32, 128, 64]) -> ##torch.Size([64, 32, 256, 128])
        z = self.relu(self.bn2(self.conv2(z))) #torch.Size([64, 32, 128, 64])   ->##torch.Size([64, 32, 256, 128])
        z = self.conv1(z) #torch.Size([64, 3, 256, 128]) 
